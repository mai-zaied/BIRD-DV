    `timescale 1ns/1ps
    module bird_m3_remote_newfrag1_tb;
        import bird_tb_pkg::*;
    ...
    bird_remote_coverage cov;
        ...
        initial begin
            ...
            $fsdbDumpvars(0, bird_m3_remote_newfrag1_tb);
        end
        ...
        initial begin
            ...
            gen  = new(1);            // SPEC mode: index on FRAG_NUM, SEQ_NUM is packet id
        cov  = new();
            ...
            // Start a packet (SEQ_NUM=5) with position 1 -> packet is now incomplete
            // Then send a NEW position-1 fragment with a DIFFERENT SEQ_NUM=6
            // Per spec: this should drop the incomplete packet.
            pa = '{8'h11, 8'h22, 8'h33};
            pb = '{8'h44, 8'h55, 8'h66};

            fa = gen.make_fragment(pa, 1, 5, 8'hA0, 8'hA1);   // position 1, SEQ_NUM 5
            fb = gen.make_fragment(pb, 1, 6, 8'hB0, 8'hB1);   // position 1 AGAIN, SEQ_NUM 6

            drv.drive_fragment(fa);
        cov.sample_fragment(fa);
            drv.drive_fragment(fb);
        cov.sample_fragment(fb);
        cov.sample_packet(2, 0, 1, bird_remote_coverage::DROP_NEW_FRAG1);

            repeat (60) @(bif.cb_drv);

            got = rword_mb.num();
            $display("[M3_NEWF1] ---- remote output words ----");
            while (rword_mb.num() > 0) begin
                rword_mb.get(w);
                $display("[M3_NEWF1] remote word = 0x%08h", w);
            end
            $display("[M3_NEWF1] total remote words = %0d", got);
            $display("[M3_NEWF1] DUT drop_cnt = %0d (spec expects a drop here)", bif.drop_cnt);

            repeat (10) @(bif.cb_drv);
        cov.report("bird_m3_remote_newfrag1_tb");
            $finish;
        end
    endmodule
